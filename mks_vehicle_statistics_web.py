...
# ---------------------------------------------------------
APP_TITLE = "Japan Customs Vehicle Export Statistics"
...
def export_result_to_excel_bytes(result: dict, monthly_df: pd.DataFrame, logo_path: Path | None = None) -> bytes:
    ...
    ws1["A1"].font = Font(size=14, bold=True, color="1F3F4F")
    ws1["A2"].font = Font(size=12, bold=True, color="529CBA")
...
def main():
    ...
    st.markdown(f"<h3 style='margin-bottom: 0px;'>Japan Vehicle Export Statistics</h3>", unsafe_allow_html=True)
    st.markdown(
    f"<p style='color: {MKS_BLUE}; font-size: 0.9em;'>Multi-year Japan Customs export data | New and Used vehicles</p>",
    unsafe_allow_html=True
)
    ...
    if subset.empty:
        ...
        st.markdown(
            ...
            f"<span style='font-size:14px;'>Showing <strong>{selected_year} / {selected_condition} / {selected_scope} / {period_selected}</strong> for <strong>{selected_country}</strong>: {quantity:,}</span>"
            f"</div>", 
            unsafe_allow_html=True
        )
        ...
        with c1:
            st.markdown(f"<h4 style='color: {MKS_DARK}; margin-bottom: 8px;'>Summary</h4>", unsafe_allow_html=True)

            st.markdown(
        f"""
        <div style="font-size: 1rem; line-height: 1.6;">
            <div><strong>Total Quantity:</strong> {quantity:,}</div>
            <div><strong>Country:</strong> {selected_country}</div>
            <div><strong>Year:</strong> {selected_year}</div>
            <div><strong>Vehicle Condition:</strong> {selected_condition}</div>
            <div><strong>Dataset:</strong> {selected_scope}</div>
            <div><strong>Period:</strong> {period_selected}</div>
            <hr style="margin:10px 0;">
            <div><strong>Steering Rule:</strong> {steering_rule}</div>
            <div><strong>Steering Notes:</strong> {steering_notes if steering_notes else '-'}</div>
            <div><strong>Source:</strong> <em>{row['source_file']}</em></div>
        </div>
        """,
        unsafe_allow_html=True
    )
        ...
        with c2:
            st.markdown(f"<h4 style='color: {MKS_DARK};'>Monthly Breakdown</h4>", unsafe_allow_html=True)
    ...
    if not st.session_state["admin_logged_in"]:
        ...
        with st.sidebar.expander("🌎 Edit Steering Rules"):
            ...
            edited_steering_df = st.data_editor(steering_df, num_rows="dynamic", width='stretch')
